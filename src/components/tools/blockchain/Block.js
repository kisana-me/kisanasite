...
export default function Block({ blockNumber, previousHash, onMine, difficulty }) {
  ...
  return (
    <div className="block">
      ...
      <label>Nonce: </label><input type="number" value={nonce} disabled /><br />
      <label>Text: </label>
      <textarea
        value={text}
      ...
      <input type="text" value={previousHash} disabled /><br />
      <label>Hash:</label><br />
      <input type="text" value={hash} disabled /><br />
      <label>Time Taken: {timeTaken ? `${timeTaken} seconds` : 'N/A'}</label><br />
